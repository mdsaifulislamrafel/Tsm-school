    ...
    return (
        <div data-aos="fade-up" data-aos-duration="2500" className="w-full mx-auto h-[240px] md:h-[540px] overflow-hidden gap-5 my-5 md:my-16">
            <div className="relative overflow-hidden">
                ...
                {/* slider container */}
                <div className="ease-linear duration-300 flex flex-col h-[540px] transform-gpu relative" style={{ transform: `translateY(-${currentSlider * 100}%)` }}>
                    {/* sliders */}
                    {sliders.map((_, inx) => (
                        <div key={inx} className="min-w-full duration-200 before:content-['Image'] before:bg-black/20 before:-z-10 before:absolute before:text-3xl before:flex before:justify-center before:items-center before:text-black/40 before:inset-0 relative">
                            <img src={_} className="w-full h-[540px] object-cover" alt={`Slider - ${inx + 1}`} />
                        </div>
                    ))}
                </div>
            </div>
